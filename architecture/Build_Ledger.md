...
| # | Task | Status |
|---|---|---|
| I1 | Private **Supabase Storage** bucket + `photos` table + RLS (approved emails only) | ❌ |
| I2 | Upload → keep original + generate thumbnail + record dims/bytes; **require ≥1 tag** | ❌ |
| I3 | **`cleared_for_public` HARD gate** (checklist: no faces/valuables/address/OK-to-show). Uncleared photos unusable anywhere. | ❌ |
| I4 | Maker-vocabulary tagging; `photo_state` chip (raw/retouched/pro, informational) | ❌ |
| I5 | Asset page: **latest-10 + most-used** default, search ranked over tags+description, missing-description floats top | ❌ |
| I6 | `use_count` / **least-used-first** rotation (ban-risk: don't repeat images) | ❌ |
...
| # | Task | Status |
|---|---|---|
| S1 | **Support** tab + `support_tickets` table + RLS | ❌ |
| S2 | New ticket (bug/feedback/feature/backlog) + list/filter/status chips | ❌ |
| S3 | AI **triage** on create (Edge Function classify/summarise/cause) | ❌ |
| S4 | **Short report** (`resolution`) on close — reads as a plain-English changelog | ❌ |
